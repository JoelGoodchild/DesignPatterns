...
void Game::OnUpdate(float DeltaTime, StateMachine* state)
{
	...
	//---------------------------------------------------------------------------------------
	//Fade in variable for the tip, begins fading in at 5 seconds
	//---------------------------------------------------------------------------------------
	if (time > 5)
...
void Game::OnDraw(Renderer2D* m_2dRenderer)
{
	...
	if (easterEgg)
	{
		...
		//---------------------------------------------------------------------------------------
		//If they are back in the window, this will ensure that the player is only allowed in the bounds of the window 
		//---------------------------------------------------------------------------------------
		if (backIn)
		{
			if (playerX > 1400)
